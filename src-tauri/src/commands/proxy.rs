...
#[tauri::command]
pub async fn get_routing_chain(state: State<'_, DynamicRosterState>) -> Result<Vec<CloudModel>, String> {
    let chain = state.fallback_chain.read().await;
    Ok(chain.clone())
}

#[tauri::command]
pub async fn set_routing_chain(state: State<'_, DynamicRosterState>, new_chain: Vec<CloudModel>) -> Result<(), String> {
...
#[tauri::command]
pub async fn get_provider_statuses(state: State<'_, ProviderHealthState>) -> Result<HashMap<String, String>, String> {
...
#[tauri::command]
pub async fn refresh_routing_chain(state: State<'_, DynamicRosterState>, app: tauri::AppHandle) -> Result<Vec<CloudModel>, String> {
    let new_chain = fetch_live_routing_chain(&app).await;
