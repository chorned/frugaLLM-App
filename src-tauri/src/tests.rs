...
fn test_is_ollama_in_paths() {
    ...
    assert_eq!(
        is_ollama_in_paths(&["/invalid/nonexistent/path/to/ollama"]),
        false
    );
}
...
fn test_compute_memory_segments_discrete_gpu_spillover() {
    ...
    assert!(segments
        .warning_message
        .contains("Model & 128k context exceed Dedicated VRAM"));
}
...
fn test_compute_memory_segments_apple_silicon_unified() {
    ...
    assert!(segments_31b
        .warning_message
        .contains("Memory exceeds available Unified Memory"));
}
...
fn test_server_status_serialization() {
    ...
    assert!(
        conflict_json.contains("Close the service currently using port [5050] and restart the app")
    );
}
...
fn test_child_process_manager_lifecycle() {
    ...
    assert!(pm.has_active_services());
    assert_eq!(
        pm.active_service_names(),
        vec!["hermes-dashboard".to_string()]
    );
...
fn test_is_openrouter_free_alias() {
    ...
    assert!(!is_openrouter_free_alias(
...
fn test_get_ollama_chat_model_selection() {
    ...
    assert_eq!(
        resolve_ollama_chat_model_from_tags(&json_with_active),
        "frugallm-active"
    );
    ...
    assert_eq!(
        resolve_ollama_chat_model_from_tags(&json_preinstalled),
        "llama3:8b"
    );
    ...
    assert_eq!(
        resolve_ollama_chat_model_from_tags(&json_empty),
        "frugallm-active"
    );
}
...
fn test_parse_agent_version() {
    ...
    assert_eq!(
        parse_agent_version("1.18.29\n"),
        Some("v1.18.29".to_string())
    );
    assert_eq!(parse_agent_version("v0.3.1"), Some("v0.3.1".to_string()));
    assert_eq!(
        parse_agent_version("OpenCode 2.0.4"),
        Some("v2.0.4".to_string())
    );
}
...
async fn test_provider_health_state_circuit_breaker_and_cooldowns() {
    ...
    // 3. 503 / Timeout puts model on 60s cooldown
    {
        ...
        m_cooldowns.insert(
            "nvidia/nemotron-3-ultra-550b:free".to_string(),
    ...
    {
        let m_cooldowns = state.model_cooldowns.read().await;
        let expiry = m_cooldowns
            .get("nvidia/nemotron-3-ultra-550b:free")
            .expect("nemotron cooldown");
        assert!(*expiry > std::time::Instant::now());
...
fn test_550b_penalty_in_ranking() {
    let nemotron_score =
        crate::model_db::MODEL_REGISTRY.get_score("nvidia/nemotron-3-ultra-550b-a55b");
...
fn test_two_phase_partition_candidates() {
    ...
    let (healthy, cooldown) = partition_candidates(
        &chain,
        &provider_cooldowns,
        &gated_models,
        &model_cooldowns,
...
fn test_evaluate_probe_response_status_codes() {
    ...
    assert_eq!(
        evaluate_probe_response(403, "PERMISSION_DENIED"),
        ProbeStepOutcome::PermissionDenied403
    );
    assert_eq!(
        evaluate_probe_response(403, "denied access"),
        ProbeStepOutcome::PermissionDenied403
    );
    assert_eq!(
        evaluate_probe_response(429, "RESOURCE_EXHAUSTED"),
        ProbeStepOutcome::RateLimited429
    );
    assert_eq!(
        evaluate_probe_response(429, "You exceeded your current quota"),
        ProbeStepOutcome::RateLimited429
    );
    assert_eq!(
        evaluate_probe_response(503, "high demand"),
        ProbeStepOutcome::ServiceUnavailable503
    );
    assert_eq!(
        evaluate_probe_response(500, "Internal error"),
        ProbeStepOutcome::OtherError("500".to_string())
    );
}
...
fn test_sanitize_diagnostic_logs() {
    ...
    assert_eq!(
        sanitize_diagnostic_logs("Clean logs without tokens"),
        "Clean logs without tokens"
    );
}
