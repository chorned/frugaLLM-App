...
# Add the orchestrated init effect
init_effect = """
  useEffect(() => {
    let isMounted = true;
    const runInit = async () => {
...
# Remove the individual duplicate invokes from the existing useEffect block (around line 1068)
content = re.sub(r"invoke\('check_hermes_status'\).*?catch\(console\.error\);", "", content, flags=re.DOTALL)
content = re.sub(r"invoke\('check_opencode_status'\).*?catch\(console\.error\);", "", content, flags=re.DOTALL)
content = re.sub(r"// Auto-detect OpenRouter\s*invoke\('get_credential', \{ service: 'openrouter' \}\).*?catch\(\(\) => \{\}\);", "", content, flags=re.DOTALL)
content = re.sub(r"// Auto-detect Ollama and its installation status via backend\s*invoke\('check_ollama_status'\).*?catch\(console\.error\);", "", content, flags=re.DOTALL)
content = re.sub(r"invoke\('detect_vram'\)\.then\(\(vram\) => \{.*?\}\)\.catch\(console\.error\);", "", content, flags=re.DOTALL)

# Also remove get_frugallm_config initial call in the first useEffect
# It was: invoke('get_frugallm_config').then((conf: any) => setFrugalConfig(conf)).catch(console.error); inside the useEffect near the top
content = content.replace("invoke('get_frugallm_config').then((conf: any) => setFrugalConfig(conf)).catch(console.error);\n  }, []);", "  }, []);")

# Conditional render TerminalLoader
# Look for the return statement
content = content.replace("return (\n    <div className=\"app-container\">", "return !isAppLoaded ? <TerminalLoader logs={initLogs} /> : (\n    <div className=\"app-container\">")
